...
static void config_server_load_cfg(char *cfg)
{
	...
	if(key == 0)
	{
		// A missing key falls back to "wpa2" (accepts WPA2 or better), not
		// "wpa3": esp_wifi's threshold.authmode treats WPA3 as a strict floor,
		// so a WPA3-only default silently refuses to associate with a
		// WPA2-only AP until the config is touched via the web UI. That is
		// the wrong failure mode for a device that must reconnect unattended.
